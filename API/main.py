...
async def calculate_crypto_price(request: PriceRequest):
    try:
        # Fixed B113: Added timeout parameter to requests.get()
        response = requests.get(
            f"{COINLAYER_BASE_URL}/live",
            params={"access_key": COINLAYER_API_KEY, "symbols": request.crypto_symbol.upper()},
...
if __name__ == "__main__":
    import uvicorn
    # Fixed B104: Changed host from "0.0.0.0" to "127.0.0.1" to avoid binding to all interfaces
